(function() {
  ...
  (function(jQuery) {
    ...
    if (typeof ANTT === "undefined" || ANTT === null) {
      ANTT = {};
    }
    if (typeof Stanbol === "undefined" || Stanbol === null) {
      Stanbol = {};
    }
    window.entityCache = {};
    ...
    ANTT.annotationSelector = jQuery.widget('IKS.annotationSelector', {
      ...
      _create: function() {
        ...
        if (this.isAnnotated()) {
          ...
          return this.options.cache.get(this.linkedEntity.uri, this, __bind(function(cachedEntity) {
            var userLang;
            userLang = window.navigator.language.split("-")[0];
            this.linkedEntity.label = _(cachedEntity.get("rdfs:label")).detect(__bind(function(label) {
              if (label.indexOf("@" + userLang) > -1) {
                return true;
              }
            }, this)).replace(/(^\"*|\"*@..$)/g, "");
            return this._logger.info("did I figure out?", this.linkedEntity.label);
          }, this));
        }
      },
      _destroy: function() {
      ...
      _createPreview: function(uri, response) {
        ...
        success = __bind(function(cacheEntity) {
          ...
          html = "";
          picSize = 100;
          depictionUrl = this._getDepiction(cacheEntity, picSize);
          if (depictionUrl) {
            html += "<img style='float:left;padding: 5px;width: " + picSize + "px' src='" + (depictionUrl.substring(1, depictionUrl.length - 1)) + "'/>";
          }
          descr = this._getDescription(cacheEntity);
          if (!descr) {
            this._logger.warn("No description found for", cacheEntity);
        ...
        for (_i = 0, _len = preferredLanguages.length; _i < _len; _i++) {
          ...
          for (_j = 0, _len2 = preferredFields.length; _j < _len2; _j++) {
            ...
            if (typeof property === "string" && entity.get(property)) {
              ...
              label = _(labelArr).detect(__bind(function(label) {
                if (label.indexOf("@" + lang) > -1) {
                  return true;
                }
              }, this));
              if (label) {
      ...
      _createSearchbox: function() {
        ...
        if (!this.entityEnhancements.length && !this.isAnnotated()) {
          setTimeout(__bind(function() {
            ...
            this.searchbox.val(label);
            return this.searchbox.autocomplete("search", label);
          }, this), 300);
        }
